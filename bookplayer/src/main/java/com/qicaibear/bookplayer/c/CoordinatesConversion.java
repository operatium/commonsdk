...
//        MyLog.d("show", designPoint.toString() + " ==>" + "(" + (Ox + x) + "," + (Oy - y) + ")");
    ...
    public float getLoactionPointX(float designX){
        //原点
        float Ox = center.x;
        //距离
        float x = sceenFitFactory.getSizeFloat(designX);
        //方向
        return Ox + x;
    }

    public float getLoactionPointY(float designY){
        //原点
        float Oy = center.y;
        //距离
        float y = sceenFitFactory.getSizeFloat(designY);
        //方向
        return Oy - y;
    }
